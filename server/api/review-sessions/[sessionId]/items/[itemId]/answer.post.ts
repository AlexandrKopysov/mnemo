...
export default defineEventHandler(async (event) => {
    ...
    const result = await prisma.$transaction(async (tx) => {
        ...
        const nextReview = calculateNextReview(item.card, body.answer)
        ...
        const nextCompletedCount = item.session.completedCount + 1

        const sessionCompleted = nextCompletedCount >= item.session.totalCards
